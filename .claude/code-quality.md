...
- `test_host/CLAUDE.md` e `components/keyer_core/CLAUDE.md` (blocchi `treecode` auto) nominano ancora `keyer_iambic` come dipendenza in-tree: dal 2026-09-06 è il submodule `Esp32KeyerTest`. `components/keyer_cwnet/CLAUDE.md` dice ancora che è `bg_task` a passare gli eventi di keying: dal 2026-09-07 il modulo consuma lo stream da sé (`cwnet_feed`, #55); e che la config viene solo da `g_config.remote`: dal 2026-09-08 il CONNECT porta `g_config.system.callsign` (#25). `components/keyer_core/CLAUDE.md` dice ancora «producer uses memory_order_acq_rel on write_idx»: dal 2026-09-08 il producer pubblica con uno store release dopo il campione (#57). Si risincronizzano con `map-tree`, non a mano.
