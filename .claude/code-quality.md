...
- `treemap.py check` (2026-09-19, found while syncing `host/` for #86): the root `CLAUDE.md` is 298 lines against a cap of 240, 261 of them human-written, which the tree-mapper's distill step would move into module files only with the maintainer's confirmation; and `components/keyer_iambic` is in the root map but no longer detected as a module since it became a submodule.
...
- `components/keyer_usb/CLAUDE.md` (auto `treecode` block) says the box enumerates as 0x303A/0x8002, serial `KEYER-IU3QEZ-001`, with three CDC-ACM interfaces: `usb_cdc.c:23-26` keeps esp_tinyusb's default descriptors, so it is 0x303A/0x4002, serial `123456`, two interfaces (`CONFIG_TINYUSB_CDC_COUNT=2`: CDC0 console, CDC1 log). Seen on the box 2026-09-19. Resynced with `map-tree`, not by hand.
- `components/keyer_console/src/commands.c`: `help <family>` (`cmd_help`) and `show` (`show_param_visitor`) format each value into `char buf[32]`, so a string past 31 bytes is shown cut: the 44-character WireGuard keys, `vpn.server_endpoint` (max_length 64). And `help vpn` never shows USAGE_VPN: `cmd_help()` tries the family first and vpn is one, so the command's usage is reachable only as `vpn ?`. Found 2026-09-19 while fixing the `usb_console_printf()` over-read.
