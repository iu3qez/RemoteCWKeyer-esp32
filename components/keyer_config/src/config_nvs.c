...
static void migrate_memory_window_params(nvs_handle_t handle) {
    ...
    uint8_t test_val;
    if (nvs_get_u8(handle, NVS_KEYER_MEM_WINDOW_START_PCT, &test_val) == ESP_OK) {
    ...
    nvs_set_u8(handle, NVS_KEYER_MEM_WINDOW_START_PCT, 0);
    nvs_set_u8(handle, NVS_KEYER_MEM_WINDOW_END_PCT, 100);
    nvs_erase_key(handle, "mem_win");
    nvs_commit(handle);
...
int config_load_from_nvs(void) {
    ...
    size_t str_len = sizeof(g_config.system.callsign);
