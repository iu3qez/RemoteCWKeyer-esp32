...
- `host/`  — POSIX platform layer + `cwnetd` station daemon + `panel/`, its read-only browser page in a separate process (host-only, no physical key/PTT backend)  → host/CLAUDE.md
