...
- **A stalled `--edges` with the serial output is a FAULT.** (session-settled: user-directed 2026-09-24, chosen over refusing a non-file `--edges` at start and over both; found by the code review of PR #100.) `serial_key()` drives the line, then `edge_write()` waits for the descriptor, which never drops a line. A reader that stops draining (a Ctrl-S in the terminal, a pipe nobody reads) would freeze the loop with the key down. With the serial output, a wait over 100 ms records an output fault: the loop stops, the port closes, and the line not written is counted as lost. The limit is on one write, never on how long the key is held: a tuning key-down writes one line and then nothing, and stays down as long as it is held (the Key Decision on key-down timeout above is unchanged). Governs R7, R8.
