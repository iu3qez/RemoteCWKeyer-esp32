...
- R8. Ogni byte arriva B ms prima della propria scadenza, quindi una FIFO vuota alla scadenza di un fronte è lo stato normale: lo stato applicato resta. Un byte che arriva dopo la propria scadenza si applica subito, l'elemento in corso si allunga del ritardo e il ritardo viene contato. Un tasto giù e nessun byte che arriva **non è un guasto e non ha scadenza**: è quello che si vede quando l'operatore tiene giù il tasto, cosa che si fa a ogni accordatura e a bassa potenza si deve fare. Che il client ci sia ancora lo dice il PING, che risponde il programma e non la mano dell'operatore: finché risponde, il tasto resta come l'ha lasciato. Quando i PING lo dichiarano morto (R16) la chiave si rilascia con tasto su e PTT giù alla coda, e quello è il fault. Un silenzio del keying non è mai il segnale: è indistinguibile fra chi tiene giù e chi è caduto, e il PING quei due li distingue. Un byte con lo stesso stato di quello applicato non produce un fronte.
...
- AE6. Covers R8, F5, R16. **Given** un key-down riprodotto e la FIFO vuota. **When** passano dieci secondi senza un byte MORSE ma il client risponde ai PING. **Then** il tasto resta giù, nessun fault, e all'arrivo del key-up l'elemento esce lungo quanto è stato tenuto. **Given** lo stesso key-down. **When** il client smette di rispondere e i PING lo dichiarano morto. **Then** la chiave si rilascia, il tasto va su e il PTT scende alla coda. **Given** lo stesso key-down. **When** il key-up arriva 30 ms dopo la propria scadenza. **Then** il tasto va su all'arrivo, senza fault, e il ritardo di 30 ms è contato.
...
- KTD3. **Riproduzione come nel codice del riferimento, non nel suo commento.** Parte al primo byte ritardato di B; B fissato all'over. Lo stato applicato resta finché non arriva il byte successivo, perché la FIFO è vuota per costruzione durante un elemento più lungo di B; un byte in ritardo allunga l'elemento e viene contato. Il motore non alza mai il tasto da sé: un tasto giù senza byte è l'accordatura, non un guasto, e il silenzio del keying non distingue chi tiene giù da chi è caduto. Quella distinzione la fa il PING, che risponde il programma e non la mano, e il rilascio arriva da lì (R16) o dal tetto dell'over. *(session-settled: user-directed — scelto sopra un tempo di grazia sul silenzio del keying: a bassa potenza tenere giù il tasto è procedura, e mezzo secondo la tagliava. Il tetto vero al key-down continuo sta fuori dal processo, nella Decision sull'uscita fisica.)* «corrupted CW timing is worse than silence» (ARCHITECTURE.md 8.1) resta e vale per il timing riprodotto. Motore in un modulo suo, `cwnet_play.[ch]`, con orologio iniettato. Governa R7, R8.
